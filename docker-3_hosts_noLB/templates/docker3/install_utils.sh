...
echo "44 2 * * 6 root /usr/bin/geoipupdate -d /opt/docker-3_hosts_noLB/production/docker3/geoip/ > /dev/null" >> /etc/crontab
echo "" >> /etc/crontab
wget http://geolite.maxmind.com/download/geoip/database/GeoLiteCity.dat.gz -P ./geoip/ && gunzip -f ./geoip/GeoLiteCity.dat.gz
/usr/bin/geoipupdate -d /opt/docker-3_hosts_noLB/production/docker3/geoip/ > /dev/null
...
yum -y install socat
mkdir -p /opt/docker-3_hosts_noLB/production/docker3/clamav && chown clamav:clamav /opt/docker-3_hosts_noLB/production/docker3/clamav
