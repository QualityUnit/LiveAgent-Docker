...
rm -rf `find ../docker* -type d | grep -v 'docker-3_hosts_noLB'` 2>/dev/null
...
if [ "$VALUE" -eq "1" ] 2>/dev/null; then
  ...
  cp $LA_LOCATION/la*.zip /opt/LiveAgent-Docker/docker-3_hosts_noLB/production/docker2/apache-fpm/la_latest.zip
  ...
  cp $LA_LOCATION/la*.zip /opt/LiveAgent-Docker/docker-3_hosts_noLB/production/docker3/apache-fpm/la_latest.zip
else
